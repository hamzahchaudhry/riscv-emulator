...
class Hart {
 ...
 private:
  ...
  std::expected<void, Trap> ExecuteShiftImmediate(
      const ShiftImmediateInstruction& instr);
  std::expected<void, Trap> ExecuteLoad(const LoadInstruction& instr,
                                        Memory& memory);
  std::expected<void, Trap> ExecuteStore(const StoreInstruction& instr,
                                         Memory& memory);
  ...
  static std::expected<void, Trap> ExecuteSystem(
      const SystemInstruction& instr);
  static constexpr u32 AddOffset(u32 base, i32 offset) {
    return base + static_cast<u32>(offset);
  }
